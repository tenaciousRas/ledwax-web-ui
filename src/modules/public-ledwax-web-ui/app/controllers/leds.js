...
angular.module('LEDWAXW3.leds', [ 'LEDWAXW3.services' ])
	.controller('LEDsCtrl', [ '$rootScope', '$scope', 'REST_IoT', 'PaginationFilteredSorted', '$timeout',
		function($rootScope, $scope, REST_IoT, PaginationFilteredSorted, $timeout) {
			...
			$scope.getDeviceList = () => {
				let prom = REST_IoT.getStoredDevices($scope.currentCloudHost.id, $scope.userSession.sessiontoken);
				prom.then((resp) => {
					if (resp.data) {
						...
						$scope.itemsStored.forEach((el) => {
							...
							el.ledwax_device_ledstrips.forEach((strip) => {
								...
								strip.setColor = (sliderId, modelValue, highValue, pointerType) => {
									REST_IoT.setColor($scope.currentCloudHost.id, $scope.userSession.sessiontoken, el.deviceId, el.ledstripsPagination.currentPage - 1, modelValue);
								};
								strip.setBrightness = (sliderId, modelValue, highValue, pointerType) => {
									REST_IoT.setBrightness($scope.currentCloudHost.id, $scope.userSession.sessiontoken, el.deviceId, el.ledstripsPagination.currentPage - 1, modelValue);
								};
								strip.setColorHoldTime = (sliderId, modelValue, highValue, pointerType) => {
									REST_IoT.setColorHoldTime($scope.currentCloudHost.id, $scope.userSession.sessiontoken, el.deviceId, el.ledstripsPagination.currentPage - 1, modelValue);
								};
								strip.setFadeTime = (sliderId, modelValue, highValue, pointerType) => {
									REST_IoT.setFadeTime($scope.currentCloudHost.id, $scope.userSession.sessiontoken, el.deviceId, el.ledstripsPagination.currentPage - 1, modelValue);
								};
								strip.setFadeMode = (modelValue) => {
									...
									if (fadeModeIdx >= 0) {
										REST_IoT.setFadeMode($scope.currentCloudHost.id, $scope.userSession.sessiontoken, el.deviceId, el.ledstripsPagination.currentPage - 1, fadeModeIdx);
								...
								strip.setDispMode = (modelValue) => {
									...
									if (dispModeIdx >= 0) {
										REST_IoT.setDispMode($scope.currentCloudHost.id, $scope.userSession.sessiontoken, el.deviceId, el.ledstripsPagination.currentPage - 1, dispModeIdx);
